...
/**
 * @description Renders the overall stadium crowd statistics HTML
 * @param {number} totalCurrent - Current fans count
 * @param {number} overallPct - Overall capacity percentage
 * @param {number} totalCapacity - Total capacity
 * @returns {string} HTML string
 */
function renderCrowdOverview(totalCurrent, overallPct, totalCapacity) {
  let html = '';
...
/**
 * @description Renders the individual stand zone cards HTML
 * @returns {string} HTML string
 */
function renderZoneCards() {
  ...
  /**
   * @description Appends a zone stand card details HTML
   * @param {Object} zone - Zone data object
   * @returns {void}
   */
  function addZoneCard(zone) {
    ...
    /**
     * @description Appends a facility tag HTML
     * @param {string} fac - Facility name
     * @returns {void}
     */
    function addFacilityTag(fac) {
      html += '<span class="zone-facility-tag">' + fac + '</span>';
    }
    zone.facilities.forEach(addFacilityTag);
    html += '</div>';
  ...
  activeZones.forEach(addZoneCard);
  html += '</div>';
  return html;
}

/**
 * @description Renders the crowd monitor refresh and chart section HTML
 * @returns {string} HTML string
 */
function renderCrowdFooter() {
  let html = '';
...
/**
 * @description Renders the complete crowd monitor layout
 * @returns {void}
 */
function renderCrowdMonitorUI() {
  const wrap = document.getElementById('crowd-content');
  if (!wrap) return;
  const totalCurrent = activeZones.reduce(
    /**
     * @description Accumulator helper for current crowd total
     * @param {number} s - Sum accumulator
     * @param {Object} z - Zone object
     * @returns {number} Sum value
     */
    function sumCurrent(s, z) {
      return s + z.currentCrowd;
    }, 0);
  const totalCapacity = activeZones.reduce(
    /**
     * @description Accumulator helper for capacity total
     * @param {number} s - Sum accumulator
     * @param {Object} z - Zone object
     * @returns {number} Sum value
     */
    function sumCapacity(s, z) {
      return s + z.capacity;
    }, 0);
  const overallPct = Math.round((totalCurrent / totalCapacity) * 100);
  let html = '';
  html += renderCrowdOverview(totalCurrent, overallPct, totalCapacity);
  html += renderZoneCards();
  html += renderCrowdFooter();
  wrap.innerHTML = html;
...
function buildCrowdMonitor() {
  ...
  if (activeZones.length === 0) {
    /**
     * @description Maps zone static configuration into active tracking state
     * @param {Object} zone - Zone static settings
     * @returns {Object} Zone tracker object
     */
    ...
    activeZones = STADIUM_DATA.zones.map(mapZone);
  }
  renderCrowdMonitorUI();
}
...
function attachCrowdListeners() {
  ...
  if (wrap) {
    ...
    /**
     * @description Binds click listener to AI ask buttons
     * @param {HTMLButtonElement} btn - The button element
     * @returns {void}
     */
    function bindAskBtn(btn) {
      btn.addEventListener('click', handleZoneAskClick);
    }
    askBtns.forEach(bindAskBtn);
  }
}
...
function handleRefreshCrowd() {
  /**
   * @description Modifies zone values with simulated variance
   * @param {Object} zone - Stand tracking zone object
   * @returns {void}
   */
  function adjustCrowdValue(zone) {
  ...
  activeZones.forEach(adjustCrowdValue);
  renderCrowdMonitorUI();
