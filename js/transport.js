...
/**
 * @description Renders peak operational congestion warning box HTML
 * @returns {string} HTML string
 */
function renderCongestionWarning() {
  let html = '';
...
/**
 * @description Renders filter chips layout HTML
 * @returns {string} HTML string
 */
function renderFilterChips() {
  ...
  /**
   * @description Appends filter chip button HTML to layout
   * @param {Object} c - Chip info object
   * @returns {void}
   */
  function addFilterChip(c) {
  ...
  chips.forEach(addFilterChip);
  html += '</div>';
  return html;
}

/**
 * @description Renders filtered options grid HTML
 * @param {Array} filteredOptions - Filtered transit choices array
 * @returns {string} HTML string
 */
function renderOptionsGrid(filteredOptions) {
  let html = '';
  html += '<div class="transport-grid animate-on-scroll">';
  /**
   * @description Appends a transport option card details HTML
   * @param {Object} opt - Transit option details object
   * @returns {void}
   */
  function addTransportCard(opt) {
  ...
  filteredOptions.forEach(addTransportCard);
  html += '</div>';
  return html;
}

/**
 * @description Renders commute carbon calculator card HTML
 * @returns {string} HTML string
 */
function renderCarbonCalculator() {
  ...
  /**
   * @description Appends option element for transit selection dropdown
   * @param {Object} opt - Transit option object
   * @returns {void}
   */
  function addTransitSelectOption(opt) {
    html += '<option value="' + opt.id + '">' + opt.icon + ' ' + opt.name + '</option>';
  }
  STADIUM_DATA.transport.options.forEach(addTransitSelectOption);
  html += '</select></div></div>';
...
/**
 * @description Renders the transportation split Google Charts section container HTML
 * @returns {string} HTML string
 */
...
/**
 * @description Renders the complete multi-modal transport hub layout
 * @returns {void}
 */
function renderTransportHubUI() {
  ...
  let html = '';
  html += renderCongestionWarning();
  html += renderFilterChips();
  html += renderOptionsGrid(filteredOptions);
  html += renderCarbonCalculator();
  html += renderTransitChart();
  wrap.innerHTML = html;
...
function attachTransportListeners() {
  ...
  /**
   * @description Binds click listener to filter chip buttons
   * @param {HTMLButtonElement} btn - The button element
   * @returns {void}
   */
  function bindChipBtn(btn) {
    btn.addEventListener('click', handleFilterClick);
  }
  chipBtns.forEach(bindChipBtn);
  const routeBtns = wrap.querySelectorAll('.route-btn');
  /**
   * @description Binds click listener to route info buttons
   * @param {HTMLButtonElement} btn - The button element
   * @returns {void}
   */
  ...
  routeBtns.forEach(bindRouteBtn);
